   ...
   [Range(0.5f, 0.9f)]
    ...
    void DrawIndicators()
    {
        foreach(Target target in targets)
        {
            ...
            if (target.m_MaxDistanceToDisplay > Target.m_NO_MAX_LIMIT_FOR_DISTANCE &&
                distanceFromCamera >= target.m_MaxDistanceToDisplay)
                continue;//skip this far target 
            ...
            if(indicator)
            {
                ...
                indicator.SetDistanceText(distanceFromCamera, target.m_TargetName); //Set the distance text for the indicator.
